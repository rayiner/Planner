...
| Mode switching | `PlannerMode` (`.tasks` / `.mail`) on `SelectionModel` with a new `SelectionField.mode`; panes are container VCs that swap children | "One more field on the selection model." Every existing observer already inspects `changedFields`, so adding fields is safe by contract. Container VCs avoid churning `NSSplitViewItem`s (autosave, tracking separators, holding priorities all stay put). |
